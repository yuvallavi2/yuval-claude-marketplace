...
**Decision:** Implement the scope captured in [`../output/briefs/CB-006-wiki-aware-skills.md`](../output/briefs/CB-006-wiki-aware-skills.md). Five structural commitments in one ADR:

**(a) Skill-wiki contract — three rules.** (1) Skills read the wiki freely — no permission, no quotas. (2) Skills write narrowly — only to the wiki files they declare in their `SKILL.md` (or command file) header comment; the declared write set is part of the contract, extending it requires a brief. (3) Skills never auto-modify canonical artifacts from wiki content alone — wiki content is *input* to a synthesis, the synthesis itself is user-confirmed. Canonical artifacts: `/code/SPIRIT.md`, briefs in `/output/briefs/`, ADRs in `/code/DECISIONS.md`, code files, persona.

**(b) Wiki-aware skills MVP — three skills, deliberately narrow.** `write-brief` reads `/wiki/goals.md` (Step 1.7), `/wiki/backlog.md` (Step 1.8), and `/wiki/ideation/*.md` (Step 3.5 References). It writes only the brief itself, including HTML-comment markers for `promote-to-code`. `refresh-spirit` reads `/wiki/spirit-signals.md` (Step 3.5, pending entries only) and folds pending signals into the prompt pre-fills; after the SPIRIT.md write succeeds, it appends a synthesized-status footer to each previously-pending signal (Step 7.5 — append-only, never delete). `promote-to-code` reads brief markers and (only when present) closes matched items in `/wiki/backlog.md` and archives consumed `/wiki/ideation/*.md` files into `/wiki/ideation/archive/` (Steps 5.5, 5.6). `init`, `init-code`, and `refresh-persona` deliberately stay wiki-blind.

**(c) `refresh-persona` stays wiki-blind.** Persona is project-agnostic (one persona for everyone); the wiki is project-specific. Reading the project wiki to refresh a global artifact would conflate altitudes. `refresh-persona` operates purely as a bundled-source-to-CLAUDE.md operation; the wiki has nothing to contribute to it.

**(d) Backlog match is LLM-judged with user confirmation, not fuzzy-string.** `write-brief` Step 1.8 uses Claude judgment to identify which active backlog items semantically match the emerging brief's title/scope. The user always confirms (or overrides) before any closure marker is written. Same model as the spirit-prompt pre-fill: surface, don't enforce. Robust to paraphrase; fails gracefully when nothing fits.

**(e) Brief markers carry promotion intent across the author → promotion handoff.** `<!-- backlog-closes: <slug1>,<slug2> -->` and `<!-- ideation-promotes: <slug-a>,<slug-b> -->` are HTML-comment markers written by `write-brief` immediately after the brief's status line, consumed by `promote-to-code` at promotion time. Either marker is omitted if its list is empty. Briefs without markers cause no wiki side-effects at promotion — the existing pre-v0.11.0 contract is the no-markers case. The user-confirmed match happens at brief-write time when context is freshest; promotion stays mechanical and auditable.

**Reasoning:** CB-005 (D-029) introduced the workshop layer, giving the wiki a generative half (vision / goals / spirit-signals / backlog / ideation) for skills to consume. Without an explicit consumption contract, three failure modes are likely: (1) read pollution — every skill grows ad-hoc reads from any wiki file, opaque dependency graph; (2) write pollution — skills mutate wiki state as a convenience, eroding the audit trail; (3) canonical drift — skills update SPIRIT.md or briefs from wiki content directly, removing the user from the loop. The three-rule contract (read freely / write narrowly / never auto-modify canonical) addresses each.

The wiki-aware MVP was sized to what CB-005's surfaces actually justify: three skills, two markers, one signal-status convention. The sparse-wiki fallback is identical across all three skills (missing/malformed file → skip silently, never crash) — projects without a populated workshop layer behave exactly like pre-v0.11.0 projects.

**Brief reference:** [`../output/briefs/CB-006-wiki-aware-skills.md`](../output/briefs/CB-006-wiki-aware-skills.md) — promoted 2026-05-04, status `promoted` post-promotion (flipped to `closed` at this commit's tag). The brief is immutable post-promotion and serves as the as-of-promotion snapshot of scope.

**Related:** [D-029](#d-029-—-wiki-2.0-workshop-layer-cb-005) (the surfaces this contract reads), [D-016](#d-016) (skill self-containment — declared writes per skill mirror declared references-folder per skill), [D-023](#d-023) (briefs are the only commitment artifact — markers don't change this; they attach wiki side-effects to the existing promotion event), [D-024](#d-024) (handshake commands are the only sanctioned cross-mode edges — wiki-aware behavior in `promote-to-code` extends the handshake's wiki side-effects without adding a new edge), [D-026](#d-026) / [D-027](#d-027) (refresh-persona pattern — the explicit choice to keep `refresh-persona` wiki-blind preserves the persona-vs-project altitude).
...
- *Fuzzy-string or tag-based matching.* Match is LLM-judged with user confirmation. Fuzzy-string is brittle to paraphrase; tags would require a tagging step at backlog-write time.
- *Cross-project wiki reads.* Skills read the local project's `/wiki/` only. Cross-project reading would conflate altitudes between projects.
- *Schema validation of wiki files.* All wiki files remain human prose. Format conventions are conventions, not enforced schemas. Malformed file → fall back to the sparse path.
- *A `--no-wiki` flag on any skill.* Feature creep. If the user wants to skip a wiki read, they delete or rename the file temporarily.
